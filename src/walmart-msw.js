...
module.exports = function(clientId, secretKey, host, version, name){

    //Initialize httpClient custom Axios Instance for WMS
    httpClient = axios.create({
        baseURL: host,
        headers: {
            'WM_SVC.NAME': name,
            'WM_QOS.CORRELATION_ID': uuid(),
            'Authorization': `Basic ${base_64(`${clientId}:${secretKey}`)}`,
        }
    });
    ...
    httpClient.interceptors.request.use(async  (config) => {
        ...
        Object.assign(config.headers,{
            'WM_SEC.ACCESS_TOKEN': `${token.token}`
        });
        return config;
        }, (error) => {    
